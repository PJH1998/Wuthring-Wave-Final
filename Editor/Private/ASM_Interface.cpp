...
void CASM_Interface::Menu_BehaviorTree()
{
	...
	ImGui::SameLine();
	if(ImGui::Button("Vector3"))
	{
	}
	ImGui::SameLine();
	if(ImGui::Button("Vector4"))
	{
	}
	ImGui::EndTable();
}
	...
	for(const auto& tLink : pInterface->m_Links)
	{
		if(tLink.mInputNodeIndex == to)
		{
			if(tLink.mOutputNodeIndex == from)
				return false;
			for(auto nLink : inNode.Transitions)
			{
				if(tLink.mInputSlotIndex == nLink.mInputSlotIndex)
					return false;
			}
		}
	}
...
const GraphEditor::Link CASM_Interface::tagBTDelegate::GetLink(GraphEditor::LinkIndex index)
{
	...
	IM_ASSERT(tLink.mOutputSlotIndex >= 0 && tLink.mOutputSlotIndex < outTmpl.mOutputCount);
