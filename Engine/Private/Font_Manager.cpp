...
//{
//	// 폰트 자체의 업데이트를 순회한다.
//	// 이로써 폰트 객체들이 자체적으로 lifetime을 계산하여 제거되도록 한다.
...
//	_float4 vTargetWorldPos		= pDesc->vTargetWorldPos;
//
//
//
//
...
const FTCUSTOM_FONT_GLYPH* CFont_Manager::Get_Glyph(const _wstring& tag, _uint code)
{
	...
	{
		if (!BakeOneGlyph(pFont, code, /*padding=*/1))
			return nullptr;
	}
...
const FTCUSTOM_FONT_GLYPH* CFont_Manager::Get_GlyphAndAdvance(const _wstring& fontTag, _uint codePoint, _uint prevCodePoint, _int& outAdvanceX)
{
	...
	if (BakeOneGlyph(pFont, codePoint, 0) == false)
		return nullptr;
	...
	const tFontGlyph* glyph = &iter->second;
