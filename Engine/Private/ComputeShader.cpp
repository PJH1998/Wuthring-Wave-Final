...
{
#pragma region 1. 호출하여 HLSL 셰이더 코드를 컴파일합니다.
    // HLSL 파일로 생성.
    ...
    // 이 때가 셰이더 파일에 대한 컴파일 타임. => 실제 실행시간은 런타임이지만, 이때 셰이더에 있는 값이 불변이 된다.
    ...
    HRESULT hr = D3DCompileFromFile(pFilePath
        ...
        , strEntryPoint.c_str() // 진입점 함수 이름
        , "cs_5_0" // 셰이더 타겟 프로파일 => 고정 (Shader 버전)
        , iHlslFlag
        , 0
...
#pragma region 2. 사용하여 컴퓨팅 쎼이더를 만듭니다. 
...
// x, y, z 는 ex) 8, 8, 1, 이면 => 8 * 8 * 1 로 할당 (총 스레드 그룹의 개수)
