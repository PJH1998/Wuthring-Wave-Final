	...
	m_SRVs { Prototype.m_SRVs }
{
	...
	// Prototype ���� Buffer�� Instance ���� Buffer�� �ٸ��� ������ nullptr üũ�� ���ݴϴ�.
...
_bool CModel::Play_Animation_CPU(const _string& strAnimationName, _float fTimeDelta, _float* pTrackPosition, _bool isBlend, _bool isRootMotion, _float fRootMotionRate)
{
	...
	// Animation ���� ��, ���� Animation ó�� KeyFrame�� Blend => ��ǻ� �Ⱦ��� ����.
...
#pragma region 1. �� ��� ��� �κ��� Compute Shader�� ���� �� ����.
	// 1. ���� �ִϸ��̼��� Track Position ������Ʈ
	//    (�ִϸ��̼� ���� ���� �Ǵ��� ���� ���� Ȱ�� ����)
	...
	*pTrackPosition = fTrackPosition;
	...
	// �ִϸ��̼� ���� CB ����ü => ���� AnimIndex�� TrackPosition�� ����.
	...
	pAnimCBInfo->iAnimindex = m_AnimationNameToIndex[strAnimationName]; /* �ִϸ��̼� �̸�(strAnimationName)�� �ش��ϴ� �ε��� */;

	m_pContext->Unmap(m_Buffers[BUFFER_ANIM_INFOCB], 0);


	// 4. Compute Shader�� ���ҽ� ���ε�
//    - ComputeShader.h/cpp�� Set �Լ����� ���
	pComputeShaderCom->Set_SRV("g_BoneHierarchy", m_SRVs[SRV_BONE_HIERARCHY]); // ���� .hlsl�� ����
	...
	pComputeShaderCom->Set_SRV("g_InverseBindPose", m_SRVs[SRV_INVERSEBIND_POSE]); // ���� .hlsl�� ����
...
#pragma endregion
	...
	// 6. �߰� ��� ����.(�ϴ� RootAnimation CombinedTransofrmationMatrix�� �״�� ����)
...
#pragma endregion
	...
	// 8. �ִϸ��̼��� �����ٸ�? Clear �۾��� �����ϰ� Animation�� Ŭ�������ݴϴ�.
	if (bIsAnimationEnd)
	{
		Clear_Animation(strAnimationName);
		return true; // �ִϸ��̼� ����
...
HRESULT CModel::Ready_Animation(const _char* pFilePath)
{
	...
	// Compute Shader ����� ���� Animation Index ����.
...
HRESULT CModel::Ready_Shared_Buffers()
{
	...
	for (const auto& Pair : m_Animations)
	{
		...
		// ���� �ε���, ����, ���ӽð�.
		animInfo.iStartChannelIndexOffset = static_cast<_uint>(vAllChannelBoneInfos.size()); // ���� Ž�� AnimInfo���� 0���� ���.
		animInfo.iNumChannels = static_cast<_uint>(pAnimation->Get_Channels().size()); // ��� ä���� ����
		...
		for (const auto& pChannel : pAnimation->Get_Channels())
		{
			...
			{
				// Ű�����ӿ� ���� ���� ����. Scale, Rotation, Translation, Ʈ�� ��ġ.
