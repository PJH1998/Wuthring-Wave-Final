...
void CShadow::Make_Matrices(const _float4* pFrustrumPoints)
{
	...
	for (_uint j = 0; j < m_iNumSplits; j++)
	{
		...
		XMStoreFloat4x4(&m_Matrices[ENUM_CLASS(D3DTS::PROJ)][j], Make_SplitProjMatrix( m_pSplitPoints[j], j )); // ?섎늿 ?꾨윭
	//	XMStoreFloat4x4(&m_Matrices[ENUM_CLASS(D3DTS::VIEW)][j], Make_SplitViewMatrix(vMaxExtents, vMinExtents, vCenterPos));
	//	XMStoreFloat4x4(&m_Matrices[ENUM_CLASS(D3DTS::PROJ)][j], Make_SplitProjMatrix(vMaxExtents, vMinExtents)); // ?섎늿 ?꾨윭?ㅽ??쇰줈 ?ъ쁺?됰젹 留뚮뱾湲?
	}
}
...
_matrix CShadow::Make_SplitProjMatrix( const vector<_float4>& SplitFrustrumPoints , _uint iIndex )
{
	...
	for (_uint i = 0; i < 8; i++)
	{
		XMStoreFloat4(&vShadowViewPoints[i], XMVector3TransformCoord(XMLoadFloat4(&SplitFrustrumPoints[i]), XMLoadFloat4x4(&m_Matrices[ENUM_CLASS( D3DTS::VIEW )][iIndex] ))); // ?섎늿 ?꾨윭?ㅽ? ??酉고뻾?щ줈 ?щ━湲?
...
void CShadow::Make_ProjMatrices(const _float4* pFrustrumPoints)
{
	...
	for (_uint j = 0; j < m_iNumSplits; j++)
	{
		...
		XMStoreFloat4x4(&m_ProjMatrices[j], Make_SplitProjMatrix(m_pSplitPoints[j])); // ?섎늿 ?꾨윭?ㅽ??쇰줈 ?ъ쁺?됰젹 留뚮뱾湲?
	}
}
*/
...
//	for (_uint i = 0; i < 8; i++)
//	{
//		XMStoreFloat4(&vShadowViewPoints[i], XMVector3TransformCoord(XMLoadFloat4(&SplitPoints[i]), XMLoadFloat4x4(&m_ViewMatrix))); // ?섎늿 ?꾨윭?ㅽ? ??酉고뻾?щ줈 ?щ━湲?
//
//		fMinX = min(fMinX, vShadowViewPoints[i].x);
//		fMaxX = max(fMaxX, vShadowViewPoints[i].x);
//
...
_float CShadow::Compute_SplitDistances(_float fCameraNear, _float fCameraFar, _uint iIndex, _uint iNumSplit, _float fLambda)
{
	...
	_float fLinearSplit = fCameraNear + ( ( fCameraFar - fCameraNear ) * ( static_cast<_float>( iIndex ) / static_cast<_float>( iNumSplit ) ) );	// Linear 蹂닿컙
	_float fLogSplit = fCameraNear * powf(fCameraFar / fCameraNear, static_cast<_float>( iIndex ) / static_cast<_float>( iNumSplit ));				// Log	 蹂닿컙

	fSplitDistance = ( fLambda * fLogSplit ) + ( 1.f - fLambda ) * fLinearSplit;			// Linear? Log Lerp
