...
StructuredBuffer<matrix_rm> g_InverseBindPoses : register(t1);
StructuredBuffer<GPU_KEYFRAME> g_AllKeyframes : register(t2);
StructuredBuffer<AnimInfo> g_AllAnimInfos : register(t3);
StructuredBuffer<GPU_CHANNELINFO> g_ChannelInfos : register(t4);
...
void CSMain(uint3 dispatchThreadID : SV_DispatchThreadID) // SV_DispatchThreadID : 전체 작업에서의 스레드 ID
{
    ...
    GPU_CHANNELINFO channel = g_ChannelInfos[channelIndex];
    ...
    for (uint k = 0; k < channel.iNumKeyframes - 1; ++k)
    {
        ...
        if (g_AllKeyframes[channel.iStartKeyframeOffset + k + 1].fTrackPosition > g_TrackPosition)
        {
