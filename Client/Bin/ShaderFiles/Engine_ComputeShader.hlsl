...
void SSAO(uint3 GruopID : SV_GroupID, uint3 DTID : SV_DispatchThreadID, uint3 GTID : SV_GroupThreadID, uint GruopIndex : SV_GroupIndex)
{
    ...
    for (int i = 0; i < iSampleSize; ++i)
    {
        ...
        float4 vSampeDir = float4(vRandomVector, 0.f); // ���� ���͸� vNormal ���� �ݱ� ���·� ����
       
        float4 vSamplePos = vViewPos + (vSampeDir * fSSAO_Radius);      // Radius ��ŭ �̵�
        vSamplePos.w = 1.f;                                                         // �̻��� ���� �尡�°� ����,,
        ...
        if (SampleDepth == 0.f || SampleDepth >= fRandomZ) // �ȱ׷����ְų�, ���� ��ġ���� �ڿ� �ִٸ�
        {
            Occlusion = 1.f;
        }
        else
        {
            ...
            float fNormalWeight = saturate(dot(vNormal.xyz, normalize(vSampleViewPos.xyz - vViewPos.xyz))); // ���� �븻�� Sample ��ġ������ ���� ����
            
            float fDistWeight = smoothstep(fSSAO_MaxDistance, 0.f, fDistance); // ���� ���̰� Ŭ���� ���ϰ�, �������� ���ϰ�    

            //if(fNormalWeight <= 0.1f)                                    // ���� �븻���� �ڿ� �ִٸ�
            //    Occlusion = 1.f;
            //else
            {
