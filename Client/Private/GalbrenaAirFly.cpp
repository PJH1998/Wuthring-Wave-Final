...
_float CGalbrenaAirFly::Approach(_float fCurrent, _float fTarget, _float fTimeDelta)
{
	if (fTimeDelta <= 0.f)
		return fCurrent;
...
void CGalbrenaAirFly::Update_AxisBlend(_float& fValue, _bool isNegativeInput, _bool isPositiveInput, _float fStep)
{
	if (isNegativeInput)
		fValue -= fStep;
...
void CGalbrenaAirFly::SetUp_FlyBlendClips()
{
...
void CGalbrenaAirFly::Update_FlyMovement(_float fTimeDelta)
{
	EGalbrenaAirFlyType eAirFlyType = static_cast<EGalbrenaAirFlyType>(m_iCurrentAnimIdx);
	if (eAirFlyType == EGalbrenaAirFlyType::XA_START)
		return;
