...
CSequenceLupa::CSequenceLupa(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CCharacter{ pDevice, pContext }
{
}

CSequenceLupa::CSequenceLupa(const CSequenceLupa& Prototype)
    : CCharacter(Prototype)
{
}

HRESULT CSequenceLupa::Initialize_Prototype()
{
...
HRESULT CSequenceLupa::Initialize_Clone(void* pArg)
{
	...
	CSequenceLupaFactory::Register_States(m_pStateMachineCom, this);
...
void CSequenceLupa::Priority_Update(_float fTimeDelta)
{
    if (!m_isActivate)
        return;
...
void CSequenceLupa::Update(_float fTimeDelta)
{
...
void CSequenceLupa::Late_Update(_float fTimeDelta)
{
	if (!m_isActivate)
		return;
...
void CSequenceLupa::Render_OutLine()
{
	Bind_Resources();
...
void CSequenceLupa::Render_Shadow()
{
...
void CSequenceLupa::Play_PartAnimation(_uint iPartType, const _string& strAnimName, _float fTimeDelta, _float* pTrackPosition, _float fRootMotionRate, _bool IsRootMotion, _bool IsRootMotionRotate, _bool IsRootMotionTranslate)
{
    switch (iPartType)
    {
	case PART_SPEAR:
		m_pLupaSpear->Play_Animation(strAnimName, fTimeDelta, pTrackPosition, fRootMotionRate, IsRootMotion, IsRootMotionRotate, IsRootMotionTranslate);
		break;
    default:
        break;
    }
...
void CSequenceLupa::PartActivate(_uint iPartType, _bool IsActive)
{
    switch (iPartType)
    {
	case PART_SPEAR:
		m_pLupaSpear->Activate(IsActive);
		break;
    default:
        break;
    }
...
void CSequenceLupa::Part_VolumeChange(_uint iPartType, _uint iVolumeIdx)
{
	switch (iPartType)
	{
	case PART_SPEAR:
		m_pLupaSpear->Change_Volume(iVolumeIdx);
		break;
	}
}

void CSequenceLupa::Part_VolumeActivate(_uint iPartType, _bool IsActive)
{
	switch (iPartType)
	{
	case PART_SPEAR:
		m_pLupaSpear->Volume_Activate(IsActive);
		break;
	}
}

void CSequenceLupa::Clear_PartAnimation(_uint iPartType, const _string& strAnimName)
{
    switch (iPartType)
    {
	case PART_SPEAR:
		m_pLupaSpear->Clear_Animation(strAnimName);
		break;
    default:
        break;
    }
...
void CSequenceLupa::Set_SocketMatrixToParts(_uint iPartType, const _string& strBoneName)
{
...
void CSequenceLupa::Sync_Position()
{
    m_pColliderCom->Sync_Position(m_pTransformCom);
}
...
void CSequenceLupa::Collider_Active(const _wstring& wStrColliderTag, _bool IsActive)
{
	...
	{
		if (var2 == TEXT("ATK"))
			iVolumeIdx = CLupaSpear::VOLUME::VOLUME_ATTACK;

		m_pLupaSpear->Change_Volume(iVolumeIdx);
		if (var3 == TEXT("ATTACK"))
			m_pLupaSpear->Change_VolumeLayer(iVolumeIdx, COLLISIONLAYER::ATTACK);
		else if (var3 == TEXT("KNOCKBACK"))
			m_pLupaSpear->Change_VolumeLayer(iVolumeIdx, COLLISIONLAYER::KNOCKBACK);
		else if (var3 == TEXT("SKILL"))
			m_pLupaSpear->Change_VolumeLayer(iVolumeIdx, COLLISIONLAYER::SKILL);

		m_pLupaSpear->Volume_Activate(IsActive);
	}
}

void CSequenceLupa::Effect_Active(const _wstring& wStrEffectTag)
{
...
void CSequenceLupa::Object_Func(const _wstring& wStrObjectTag)
{
	// 3개의 변수 준비
	_wstring var1, var2, var3;
...
void CSequenceLupa::Bind_ChangeEffect()
{
	PREFAB_INFO effecInfo{};
	effecInfo.pMatrixPtr = m_pTransformCom->Get_WorldMatrixPtr();
	effecInfo.pModelPtr = m_pModelCom;
...
void CSequenceLupa::Bind_DissolveTimer()
{
	Add_Condition(ENUM_CLASS(CHARACTER_CONDITION::DISSOLVE));
	m_fDissolveTimer = 0.f;
}
void CSequenceLupa::Bind_DefaultShaderPath()
{
	// 기본 Shader Path
	_uint iNumMesh = m_pModelCom->Get_NumMesh();
	for (_uint i = 0; i < iNumMesh; ++i)
...
void CSequenceLupa::Bind_DissolveShaderPath()
{
	_uint iNumMesh = m_pModelCom->Get_NumMesh();
	for (_uint i = 0; i < iNumMesh; ++i)
		m_ShaderPaths[i] = ENUM_CLASS(SHADER_ANIMMESH::DISSOLVE_NORMAL);
}
void CSequenceLupa::Activate(_bool IsActivate)
{
	...
	if (true == IsActivate)
	{
		...
		m_pStateMachineCom->Change_State(ENUM_CLASS(EStateCategory::GROUND), ENUM_CLASS(ESequenceLupaGroundState::SKILL));
	}
}
#pragma endregion
...
void CSequenceLupa::Update_Physics(_float fTimeDelta)
{
...
void CSequenceLupa::Update_TargetDistance(_float fTimeDelta)
{
...
void CSequenceLupa::Bind_Resources()
{
	_bool IsDissolve = Check_AnyCondition(ENUM_CLASS(CHARACTER_CONDITION::DISSOLVE));
	if (IsDissolve)
	{
...
void CSequenceLupa::Ready_Components(const CHARACTER_DESC* pDesc)
{
    // 1. Components
...
void CSequenceLupa::Ready_Variables(const CHARACTER_DESC* pDesc)
{
...
void CSequenceLupa::Ready_Positions(const CHARACTER_DESC* pDesc)
{
...
void CSequenceLupa::Ready_PartObjects(const CHARACTER_DESC* pDesc)
{
    _float3 vScale = {};
    _float3 vRotation = {};
    _float3 vPosition = {};
    ...
    for (_uint i = 0; i < PARTTYPE::TYPE_END; ++i)
    {
        ...
        switch (i)
        {
		case PARTTYPE::PART_SPEAR:
			vScale = { 1.f, 1.f, 1.f };
			vPosition = { 0.f, 0.f, 0.f };
			Desc = SeqPlayerData::GetLupaSpearCloneData(vScale, vRotation, vPosition, m_eCurLevel);
			...
			m_pLupaSpear = dynamic_cast<CLupaSpear*>(Find_PartObject(strPartName));
			ASSERT_CRASH(m_pLupaSpear);
			Safe_AddRef(m_pLupaSpear);
			break;
		}
    }
}

void CSequenceLupa::Ready_AttackVolumes()
{
...
CSequenceLupa* CSequenceLupa::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CSequenceLupa* pInstance = new CSequenceLupa(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed to Create : CSequenceLupa");
...
CGameObject* CSequenceLupa::Clone(void* pArg)
{
    CSequenceLupa* pInstance = new CSequenceLupa(*this);

    if (FAILED(pInstance->Initialize_Clone(pArg)))
    {
        MSG_BOX("Clone Failed : CSequenceLupa");
        Safe_Release(pInstance);
    }
