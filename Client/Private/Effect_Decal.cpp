...
HRESULT CEffect_Decal::Initialize_Prototype(const DECAL_DESC* pDesc)
{
	m_tDesc = *pDesc;
...
HRESULT CEffect_Decal::Initialize_Clone(void* pArg)
{
    ...
    m_LifeTime = m_tDesc.LifeTime;
	m_wstrMyTag = m_tDesc.wstrDecalTag;
	m_fBlendTime = m_tDesc.fBlendTime;
	m_fEmissiveIntensity = m_tDesc.fEmissiveIntensity;
...
CEffect_Decal* CEffect_Decal::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const DECAL_DESC* pDesc)
{
