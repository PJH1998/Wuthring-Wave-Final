...
CYuno::CYuno(const CYuno& Prototype)
    : CCharacter(Prototype)
{
}

HRESULT CYuno::Initialize_Prototype()
{
...
HRESULT CYuno::Initialize_Clone(void* pArg)
{
...
void CYuno::Priority_Update(_float fTimeDelta)
{
    if (!m_isActivate)
        return;
...
void CYuno::Update(_float fTimeDelta)
{
...
void CYuno::Play_PartAnimation(_uint iPartType, const _string& strAnimName, _float fTimeDelta, _float* pTrackPosition, _float fRootMotionRate, _bool IsRootMotion, _bool IsRootMotionRotate, _bool IsRootMotionTranslate)
{
    switch (iPartType)
    {
	case PART_MOON:
...
void CYuno::PartActivate(_uint iPartType, _bool IsActive)
{
    switch (iPartType)
    {
    case PART_MOON:
...
void CYuno::Part_VolumeChange(_uint iPartType, _uint iVolumeIdx)
{
	switch (iPartType)
	{
	case PART_MOON:
...
void CYuno::Part_VolumeActivate(_uint iPartType, _bool IsActive)
{
	switch (iPartType)
	{
	case PART_MOON:
...
void CYuno::Clear_PartAnimation(_uint iPartType, const _string& strAnimName)
{
    switch (iPartType)
    {
    case PART_MOON:
...
void CYuno::Set_SocketMatrixToParts(_uint iPartType, const _string& strBoneName)
{
...
void CYuno::Effect_Active(const _wstring& wStrEffectTag)
{
...
void CYuno::Object_Func(const _wstring& wStrObjectTag)
{
	// 3개의 변수 준비
	_wstring var1, var2, var3;
...
void CYuno::OnHitEnter(_uint iLayer, void* pOther, const ContactManifold& Manifold)
{
...
void CYuno::Ready_Components(const CHARACTER_DESC* pDesc)
{
    // 1. Components
...
void CYuno::Ready_Variables(const CHARACTER_DESC* pDesc)
{
...
void CYuno::Ready_Positions(const CHARACTER_DESC* pDesc)
{
...
void CYuno::Ready_AttackVolumes()
{
...
CYuno* CYuno::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CYuno* pInstance = new CYuno(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed to Create : CYuno");
...
CGameObject* CYuno::Clone(void* pArg)
{
    CYuno* pInstance = new CYuno(*this);

    if (FAILED(pInstance->Initialize_Clone(pArg)))
    {
        MSG_BOX("Clone Failed : CYuno");
        Safe_Release(pInstance);
    }
