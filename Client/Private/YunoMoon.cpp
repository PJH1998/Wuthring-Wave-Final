...
CYunoMoon::CYunoMoon(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CProp{ pDevice, pContext }
{
}

CYunoMoon::CYunoMoon(const CPartObject& Prototype)
    : CProp(Prototype )
{
}

HRESULT CYunoMoon::Initialize_Prototype()
{
...
HRESULT CYunoMoon::Initialize_Clone(void* pArg)
{
...
void CYunoMoon::Priority_Update(_float fTimeDelta)
{
...
void CYunoMoon::Update(_float fTimeDelta)
{
...
void CYunoMoon::Late_Update(_float fTimeDelta)
{
...
void CYunoMoon::Activate(_bool IsActivate)
{
...
void CYunoMoon::Change_Volume(_uint iVolumeIdx)
{
...
void CYunoMoon::Change_VolumeLayer(_uint iVolumeIdx, COLLISIONLAYER eLayer)
{
	if (m_AttackVolumes[iVolumeIdx] != nullptr)
		m_AttackVolumes[iVolumeIdx]->Change_Layer(eLayer);
}

void CYunoMoon::OnHitEnter(_uint iLayer, void* pOther, const ContactManifold& Manifold)
{
}



void CYunoMoon::Ready_Components(const PROP_DESC* pDesc)
{
    // 1. Components
...
void CYunoMoon::Ready_Variables(const PROP_DESC* pDesc)
{
...
void CYunoMoon::Ready_Positions(const PROP_DESC* pDesc)
{
...
void CYunoMoon::Ready_AttackVolumes()
{
	// size 설정
	...
	TriggerDesc.eDamageType = TEXT_COLOR_TYPE::AERO;
...
CYunoMoon* CYunoMoon::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CYunoMoon* pInstance = new CYunoMoon(pDevice, pContext);
    if (FAILED(pInstance->Initialize_Prototype()))
    {
        Safe_Release(pInstance);
        MSG_BOX("Create Failed CYunoMoon");
    }
    return pInstance;
}

CGameObject* CYunoMoon::Clone(void* pArg)
{
    CYunoMoon* pInstance = new CYunoMoon(*this);
    if (FAILED(pInstance->Initialize_Clone(pArg)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Clone Failed CYunoMoon");
    }
    return pInstance;
}
