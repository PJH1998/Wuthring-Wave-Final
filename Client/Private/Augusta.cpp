...
void CAugusta::Hit_Judge(void* pArg)
{
	...
	if (!IsAttack)
		...
		m_pGameInstance->Change_TimeRatio_ToLayer(ENUM_CLASS(m_eCurLevel), TEXT("Layer_Players"), 0.5f, m_fDodgeableHitTimer); // Dodge 시간 동안 느리게하기?
		m_pGameInstance->Change_TimeRatio_ToLayer(ENUM_CLASS(m_eCurLevel), TEXT("Layer_Enemy"), 0.5f, m_fDodgeableHitTimer); // Dodge 시간 동안 느리게하기?
