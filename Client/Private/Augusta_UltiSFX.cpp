...
CAugusta_UltiSFX::CAugusta_UltiSFX(const CAugusta_UltiSFX& Prototype)
	...
	, m_vSFXLutIntensity{ Prototype.m_vSFXLutIntensity }
{
}
...
HRESULT CAugusta_UltiSFX::Initialize_Prototype()
{
	...
	m_iSFXLutIndex = 3;
	m_vSFXLutIntensity = _float2(1.f, 0.f);
...
void CAugusta_UltiSFX::Update(_float fTimeDelta)
{
	...
	m_fCurrentLutIntensity = lerp(m_vSFXLutIntensity.y, m_vSFXLutIntensity.x, fRatio);

	_float fCurSizeY = m_vScale.y * fRatio;
	_float fSizeY = max(fCurSizeY, 0.1f);

	Setting_Scale((m_vScale.x), fSizeY);
	m_pGameInstance->Setting_LUT(m_iSFXLutIndex, m_fCurrentLutIntensity, true);
}
...
void CAugusta_UltiSFX::Reset(const _fmatrix& WorldMatrix, void* pArg)
{
	m_isActivate = true;
	m_fCurrentTime = 0.f;
	m_fCurrentLutIntensity = m_vSFXLutIntensity.x;

	Setting_Scale(m_vScale.x, m_vScale.y);

	m_pGameInstance->Get_Current_LutSetting(&m_iPrevLutIndex, &m_fPrevLutIntensity, &m_PrevIsDynamicLUT);

	m_vSFXLutIntensity.y = m_fPrevLutIntensity;

	m_pGameInstance->Setting_LUT(m_iSFXLutIndex, m_fCurrentLutIntensity, true);
}

HRESULT CAugusta_UltiSFX::Ready_Components()
{
