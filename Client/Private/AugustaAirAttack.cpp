...
void CAugustaAirAttack::Check_StateTransition(_float fTimeDelta)
{
    ...
    if (IsEscapePossible)
    {
        ...
        {
			// 그리폰 스킬 하늘 진행중.
			if (eAirAttackType == EAugustaAirAttackType::AIRATTACK_HACKDOWN_LOOP) // Root에서 닿으면?
			{
    ...
    if (m_IsAnimationEnd)
    {
        ...
        if (eAirAttackType == EAugustaAirAttackType::AIRATTACK_HACKDOWN_START)
        {
			...
			if (!m_States[LAND])
			{
				m_pAugusta->GetStateContextForWrite().m_eAirAttackType = EAugustaAirAttackType::AIRATTACK_HACKDOWN_LOOP;
				m_pAugusta->GetStateContextForWrite().m_strPrevInfo = "Griffon";
				m_pAugusta->Change_State(ENUM_CLASS(EStateCategory::AIR), ENUM_CLASS(EAugustaAirState::AIR_ATTACK));
				return;
			}
        ...
        if (!m_States[LAND])
        {
			...
			if (eAirAttackType == EAugustaAirAttackType::AIRATTACK_LOOP)
			{
				m_pAugusta->GetStateContextForWrite().m_eAirAttackType = EAugustaAirAttackType::AIRATTACK_LOOP;
				m_pAugusta->Change_State(ENUM_CLASS(EStateCategory::AIR), ENUM_CLASS(EAugustaAirState::AIR_ATTACK)); // 상위, 하위 상태
				return;
			}
			else if (eAirAttackType == EAugustaAirAttackType::AIRATTACK_HACKDOWN_LOOP)
			{
				m_pAugusta->GetStateContextForWrite().m_eAirAttackType = EAugustaAirAttackType::AIRATTACK_HACKDOWN_LOOP;
				m_pAugusta->Change_State(ENUM_CLASS(EStateCategory::AIR), ENUM_CLASS(EAugustaAirState::AIR_ATTACK)); // 상위, 하위 상태
				return;
...
void CAugustaAirAttack::SetUp_Animations()
{
    ...
    CState::Add_Animations(ENUM_CLASS(EAugustaAirAttackType::AIRATTACK_HACKDOWN_START),"AirAttack_HackDown_Start", 1.5f, 20.f, 1.2f);
    CState::Add_Animations(ENUM_CLASS(EAugustaAirAttackType::AIRATTACK_HACKDOWN_LOOP),"AirAttack_HackDown_Loop", 1.5f, 0.f, 1.2f);
    CState::Add_Animations(ENUM_CLASS(EAugustaAirAttackType::AIRATTACK_HACKDOWN_SP_END),"AirAttack_HackDown_Sp_End", 1.5f, 20.f, 1.2f);
    ...
    CState::Add_Animations(ENUM_CLASS(EAugustaAirAttackType::AIRATTACK_END),"AirAttack_End", 1.5f, 10.f, 1.f);
