...
NS_BEGIN(Client)
class CLupaSpear final : public CProp
{
public:
	enum VOLUME
	{
...
protected:
	explicit CLupaSpear(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit CLupaSpear(const CPartObject& Prototype);
	virtual ~CLupaSpear() = default;
...
public:
	static CLupaSpear* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
