...
NS_BEGIN(Client)
class CRoverDarkScythe final : public CProp
{
public:
	enum VOLUME
	{
...
protected:
	explicit CRoverDarkScythe(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit CRoverDarkScythe(const CPartObject& Prototype);
	virtual ~CRoverDarkScythe() = default;
...
public:
	static CRoverDarkScythe* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
