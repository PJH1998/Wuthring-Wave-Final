...
#include "SequenceLupaState_Enum.h"

NS_BEGIN(Client)
class CSequenceLupa final : public CCharacter
{
public:
	enum VOLUME
	{
...
private:
	...
	{
		// Prev Info
		ESequenceLupaSkillType m_eSkillType = ESequenceLupaSkillType::END;
		_string m_strPrevInfo = {};
		void Clear()
		{
			m_eSkillType = ESequenceLupaSkillType::END;
			m_strPrevInfo.clear();
		};
	};
...
#pragma region 0. 
private:
	explicit CSequenceLupa(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit CSequenceLupa(const CSequenceLupa& Prototype);
	virtual ~CSequenceLupa() = default;
...
private:
	...
	class CLupaSpear* m_pLupaSpear = { nullptr };
...
public:
	static		CSequenceLupa* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
