...
#include "SequenceAugustaState_Enum.h"

NS_BEGIN(Client)
class CSequenceAugusta final : public CCharacter
{
public:
	enum VOLUME
	{
...
private:
	...
	{
		// Prev Info
		ESequenceAugustaSkillType m_eSkillType = ESequenceAugustaSkillType::END;
		_string m_strPrevInfo = {};
		void Clear()
		{
			m_eSkillType = ESequenceAugustaSkillType::END;
			m_strPrevInfo.clear();
		};
	};
...
#pragma region 0. 
private:
	explicit CSequenceAugusta(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit CSequenceAugusta(const CSequenceAugusta& Prototype);
	virtual ~CSequenceAugusta() = default;
...
public:
	static		CSequenceAugusta* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
